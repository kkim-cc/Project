...
namespace WMP_Project
{
    ...
    public sealed partial class Success : Page
    {
        ...
        public Success()
        {
            ...
            nGameTime = MainPage.Current.GetElapsedTime();
            if (nGameTime >= 60)
            ...
            lbErr.Foreground = new SolidColorBrush(Colors.DarkOrange);
        }
        ...
        {
            // save to file.. go to main window..
            if (ebUserNmae.Text == "")
            {
                lbErr.Text = "Error: Enter User Name...";
                return;
            }

            if (ebUserNmae.Text.Length > 40)
            {
                lbErr.Text = "Error: Too long... Enter a User Name less than 40 characters.";
                return;
            }

            lbErr.Text = "";

            await MainPage.ReadTextFile();

            string strData = String.Format("{0}|{1}\n", ebUserNmae.Text, nGameTime.ToString());

            await MainPage.WriteTextFile(strData);
